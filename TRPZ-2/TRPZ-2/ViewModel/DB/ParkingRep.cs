...
namespace TRPZ_2.ViewModel.DB
{
    class ParkingRep : IRepository<Parking>
    {
        ...
        public async Task Create(Parking item)
        {
            //3
            await Task.Run(() => {
                byte[] data = new Converter<ModelsForWpf.Parking>().ObjectToByteArray(item);
        ...
        public async Task<Parking> GetItem(int id)
        {//2
            return await Task.Run(() => {
                var item = new Parking();
                item.Id = id;
                byte[] data = new Converter<Parking>().ObjectToByteArray(item);
                var toSend = new byte[data.Length + 1];
                toSend[0] = 35;
                for (int i = 0; i < data.Length; i++)
                    toSend[i + 1] = data[i];
                ...
                try
                {
                    var ret = new Converter<Parking>().ByteArrayToObject(data);
                    return ret;
                }
                catch (Exception ex)
                {
        ...
        public async Task<IEnumerable<Parking>> GetItems()
        {//1
            return await Task.Run(() => {
                ...
                try
                {
                    var ret = new Converter<List<Parking>>().ByteArrayToObject(data);
                    return ret;
                }
                catch (Exception ex)
                {
